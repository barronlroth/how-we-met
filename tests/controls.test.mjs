...
import { createRace, stepRace, horn } from '../florida/core.js';
...
test('multiple fingers retain independent ownership when one is released', () => {
  ...
  assert.deepEqual(drivingInput(new Set(), input.state), { steer: 1, boost: true, brake: true });
...
test('reset discards fingers and ignores late events after pause, restart, or rotation', () => {
  ...
  assert.deepEqual(drivingInput(new Set(), input.state), { steer: 0, brake: false, boost: false });
  assert.equal(input.begin(3, 'steer', pad), true);
});

test('keyboard keeps working alongside touch without doubling steering', () => {
  assert.deepEqual(drivingInput(new Set(['ArrowLeft', 'ShiftRight']), { steer: .6, brake: true, boost: false }), { steer: -1, brake: true, boost: true });
  assert.equal(drivingInput(new Set(), { steer: .6, brake: false, boost: false }).steer, .6);
});

test('touch inputs drive the real simulation, replenish cafecito on drift release, and respect horn cooldown', () => {
  ...
  assert.equal(horn(race), true); assert.equal(horn(race), false);
});
...
function controlsFixture() {
  const win = new EventTarget(), elements = ['steer', 'brake', 'boost', 'horn'].map(name => new Control(name));
  const root = { ownerDocument: { defaultView: win }, querySelector: () => elements[0], querySelectorAll: () => elements.slice(1) };
  let active = true, honks = 0;
  const controls = bindTouchControls(root, { active: () => active, onHorn: () => honks++ });
  const down = (element, id) => element.dispatchEvent(event('pointerdown', { pointerId: id, pointerType: 'touch', clientX: 110, clientY: 60 }));
  return { win, elements, controls, down, deactivate: () => { active = false; }, honks: () => honks };
}
test('DOM binding captures each finger, cancels independently, and resets captures safely', () => {
  const f = controlsFixture(), [steer, , boost, hornButton] = f.elements;
  f.down(steer, 1); f.down(boost, 2); f.down(hornButton, 3);
  assert.equal(f.honks(), 1); assert.equal(f.controls.state.boost, true); assert.ok(f.controls.state.steer > .5);
  ...
  assert.equal(boost.hasPointerCapture(4), false); assert.equal(hornButton.hasPointerCapture(3), false);
  assert.equal(f.controls.state.boost, false);
