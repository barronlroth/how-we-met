...
import {makeCombinedOutputPass} from './postprocessing.js';
import {floater,flamingo,gator,pickup,ramp,boatWake,C} from './art.js';
...
import {bindTouchControls,drivingInput} from './controls.js';
...
function preference(key,fallback,choices){try{const value=storage.getItem(key);return choices.includes(value)?value:fallback}catch{return fallback}}
...
function syncControls(){
 ...
 for(const name of ['start','pause']){$(`controls-${name}`).value=controlsMode;$(`graphics-${name}`).value=graphicsMode}
 $('touch-controls').hidden=controlsMode!=='touch'||race.demo||!['racing','countdown'].includes(race.status);
}
function setControls(mode,remember=true){controlsMode=mode;resetInput();syncControls();cameraSnap=true;if(remember){explicitControls=true;try{storage.setItem('florida-controls',mode)}catch{}}}
function applyGraphics(){
 if(!renderer)return;
 const profile=graphicsProfile(graphicsMode,devicePixelRatio),benchmarkDpr=Number(new URLSearchParams(location.search).get('benchmarkDpr'));
 renderer.setPixelRatio([1,1.25].includes(benchmarkDpr)?benchmarkDpr:profile.pixelRatio);
...
for(const name of ['start','pause']){
 $(`controls-${name}`).addEventListener('change',event=>setControls(event.target.value));
 $(`graphics-${name}`).addEventListener('change',event=>{graphicsMode=event.target.value;try{storage.setItem('florida-graphics',graphicsMode)}catch{}syncControls();applyGraphics()});
}
// A touch on a hybrid device reveals the controls without disabling its keyboard.
window.addEventListener('pointerdown',event=>{if(event.pointerType==='touch'&&!explicitControls&&controlsMode!=='touch')setControls('touch',false)},{passive:true});
coarsePointer.addEventListener('change',event=>{if(event.matches&&!explicitControls)setControls('touch',false)});
syncControls();
...
window.addEventListener('keydown',event=>{
 ...
 // Native controls own their editing keys. Tab still reaches the dialog trap.
 if(event.target.closest?.('select,input,textarea')&&event.code!=='Tab')return;
...
function renderEntities(s){
 for(const o of race.objects){const mesh=entities.get(o.id);if(!mesh)continue;mesh.visible=o.s>s-80&&o.s<s+650&&!o.consumed&&!o.destroyed&&(!(race.status==='ready'||race.status==='finished')||o.s>s+65);if(!mesh.visible)continue;const p=pointAt(o.s,objectX(o,race.elapsed));mesh.position.set(p.x,0,p.z);mesh.rotation.set(0,-p.heading,0);
...
function frame(now){
 ...
 const floatingPoint=pointAt(s+65,7);openingFlamingo.visible=staged;openingFlamingo.position.set(floatingPoint.x,.35+(reducedMotion?0:Math.sin(time*1.2)*.07),floatingPoint.z);openingFlamingo.rotation.y=-floatingPoint.heading-.55;
 const bob=reducedMotion?0:Math.sin(time*3)*.025+Math.sin(time*7)*race.speed*.00065;
 boat.scale.setScalar(staged&&innerWidth>=1100?1.18:1);boat.position.set(p.x,(staged?0:race.y)+bob,p.z);boat.rotation.y=-heading+(staged?Math.PI+.18:0);boat.rotation.z=reducedMotion||staged?0:T.MathUtils.lerp(boat.rotation.z,-race.turn*.19,1-Math.exp(-dt*8));boat.rotation.x=reducedMotion||staged?0:T.MathUtils.lerp(boat.rotation.x,race.y>0?-race.vy*.018:-Math.min(.065,race.speed*.001)+Math.sin(time*8)*.004,1-Math.exp(-dt*8));
 ...
 const compact=innerWidth<1100,phone=innerWidth<600,portrait=controlsMode==='touch'&&innerHeight>innerWidth;
 const fx=Math.sin(smoothedHeading),fz=-Math.cos(smoothedHeading),nx=Math.cos(smoothedHeading),nz=Math.sin(smoothedHeading),back=staged?(phone?14:compact?11.5:9.0):portrait?(race.boosting?11.2:12.4):race.boosting?8.2:9.4,height=staged?(phone?11:compact?8.5:6.0):portrait?(race.boosting?7.8:8.6):race.boosting?5.4:6.4,side=staged?(phone?3:compact?.6:3.8):portrait?.6:3.8;
 desiredCamera.set(p.x-fx*back+nx*side,height+(staged?0:race.y*.58),p.z-fz*back+nz*side);lookAt.set(p.x+fx*(staged?(phone?7:12):portrait?11:16)+nx*(staged?(phone?-.8:compact?3.5:0):portrait?0:1.6),(staged?(phone?-2:compact?-1:-.8):.4)+race.y*.35,p.z+fz*(staged?(phone?7:12):portrait?11:16)+nz*(staged?(phone?-.8:compact?3.5:0):portrait?0:1.6));
 portraitFill.visible=staged;portraitFill.position.copy(desiredCamera);
 if(staged){camera.setViewOffset(innerWidth,innerHeight,-innerWidth*(phone?.03:compact?.18:finished?.20:.20),innerHeight*(phone?(controlsMode==='touch'&&innerHeight<700?.10:.16):0),innerWidth,innerHeight)}else if(portrait){camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.10,innerWidth,innerHeight)}else camera.clearViewOffset();
 ...
 scenery.update(s,camera,staged);water.update(s,time);renderEntities(s);effects.update(race,dt,time);targetHealth.update(race,camera);
...
// Geometry is antialiased in the composer target; the canvas only receives
// a fullscreen resolved image, so a second canvas MSAA buffer is redundant.
async function boot(){
 await document.fonts.ready;await prepareMaterials();await Promise.all([loadArtMaterials(),loadHeroArt(),loadWaterfrontArt(),loadVegetationArt(),loadLandmarkArt(),loadSkyArt(),loadWaterArt()]);renderer=new T.WebGLRenderer({canvas:$('world'),antialias:false,powerPreference:'high-performance'});applyGraphics();renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;renderer.shadowMap.enabled=true;renderer.shadowMap.autoUpdate=false;renderer.shadowMap.type=T.PCFShadowMap;renderer.info.autoReset=false;
 scene=new T.Scene();scene.fog=new T.Fog(0xb9d0dc,650,2350);camera=new T.PerspectiveCamera(63,innerWidth/innerHeight,.15,6000);resize();scene.add(new T.HemisphereLight(0xd7e8ff,0x555844,.65));
 sunshine=new T.DirectionalLight(0xffdbad,3.8);sunshine.castShadow=true;sunshine.shadow.mapSize.set(2048,2048);Object.assign(sunshine.shadow.camera,{left:-100,right:100,top:100,bottom:-100,near:1,far:280});sunshine.shadow.normalBias=.045;sunshine.shadow.bias=-.00008;sunshine.shadow.radius=2;scene.add(sunshine,sunshine.target);
 portraitFill=new T.PointLight(0xffe3c8,55,24,2);portraitFill.visible=false;scene.add(portraitFill);
 ...
 $('world').addEventListener('webglcontextlost',event=>{event.preventDefault();if(['racing','countdown'].includes(race.status))pauseRace();resetInput();$('touch-controls').hidden=true;$('paused').hidden=true;$('error-message').textContent='The graphics connection was interrupted. Reload to relaunch the boat.';$('error').hidden=false});
}
