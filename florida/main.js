...
import {loadWaterfrontArt,bindWaterfrontEnvironment} from './waterfront-art.js';
...
function applyGraphics(){
 if(!renderer)return;
 const query=new URLSearchParams(location.search),profile=graphicsProfile(graphicsMode,devicePixelRatio),benchmarkDpr=Number(query.get('benchmarkDpr')),benchmarkAA=Number(query.get('benchmarkAA'));
 if([2,4].includes(benchmarkAA))profile.samples=benchmarkAA;
...
function frame(now){
 ...
 const compact=innerWidth<1100,phone=innerWidth<600,shortPhone=phone&&innerHeight<700,touchLandscape=controlsMode==='touch'&&innerWidth>innerHeight&&innerHeight<520,portrait=controlsMode==='touch'&&innerHeight>innerWidth;
 const fx=Math.sin(smoothedHeading),fz=-Math.cos(smoothedHeading),nx=Math.cos(smoothedHeading),nz=Math.sin(smoothedHeading),back=staged?(touchLandscape?8.7:shortPhone?17:phone?12:compact?11.5:9.0):portrait?(race.boosting?11.2:12.4):race.boosting?8.2:9.4,height=staged?(touchLandscape?5.2:shortPhone?8:phone?6.5:compact?8.5:6.0):portrait?(race.boosting?7.8:8.6):race.boosting?5.4:6.4,side=staged?(touchLandscape?2.8:phone?3:compact?.6:3.8):portrait?.6:3.8;
 desiredCamera.set(p.x-fx*back+nx*side,height+(staged?0:race.y*.58),p.z-fz*back+nz*side);lookAt.set(p.x+fx*(staged?(phone?10:12):portrait?11:16)+nx*(staged?(touchLandscape?0:phone?-.8:compact?3.5:0):portrait?0:1.6),(staged?(phone?-.7:compact?-1:-.8):.4)+race.y*.35,p.z+fz*(staged?(phone?10:12):portrait?11:16)+nz*(staged?(touchLandscape?0:phone?-.8:compact?3.5:0):portrait?0:1.6));
 portraitFill.visible=staged;portraitFill.position.copy(desiredCamera);
 if(staged){camera.setViewOffset(innerWidth,innerHeight,-innerWidth*(touchLandscape?.12:phone?.03:compact?.18:finished?.20:.20),innerHeight*(phone?(shortPhone?.16:.12):0),innerWidth,innerHeight)}else if(portrait){camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.10,innerWidth,innerHeight)}else camera.clearViewOffset();
