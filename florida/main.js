...
function frame(now){
 ...
 const fx=Math.sin(smoothedHeading),fz=-Math.cos(smoothedHeading),nx=Math.cos(smoothedHeading),nz=Math.sin(smoothedHeading),back=staged?(phone?14:compact?12.5:8.5):portrait?(race.boosting?11.2:12.4):race.boosting?8.2:9.4,height=staged?(phone?11:compact?9:7.2):portrait?(race.boosting?7.8:8.6):race.boosting?5.4:6.4,side=staged?(phone?3:compact?.6:3.8):portrait?.6:3.8;
 desiredCamera.set(p.x-fx*back+nx*side,height+(staged?0:race.y*.58),p.z-fz*back+nz*side);lookAt.set(p.x+fx*(staged?(phone?7:12):portrait?11:16)+nx*(staged?(phone?-.8:compact?3.5:0):portrait?0:1.6),(staged?(phone?-2:compact?-1:-1.6):.4)+race.y*.35,p.z+fz*(staged?(phone?7:12):portrait?11:16)+nz*(staged?(phone?-.8:compact?3.5:0):portrait?0:1.6));
 portraitFill.visible=staged;portraitFill.position.copy(desiredCamera);
 if(staged){camera.setViewOffset(innerWidth,innerHeight,-innerWidth*(phone?.03:compact?.18:finished?.15:.14),innerHeight*(phone?(controlsMode==='touch'&&innerHeight<700?.10:.16):0),innerWidth,innerHeight)}else if(portrait){camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.10,innerWidth,innerHeight)}else camera.clearViewOffset();
