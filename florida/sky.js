...
// The same angular remapping is used by the visible dome and water's rough
// reflection lobe. It gives the original panorama separate cloud groups at
// this camera's low elevation without filtering them through a small cube.
...
export function makeSky(scene,renderer) {
  ...
  const material=new T.ShaderMaterial({
    uniforms:{panorama:{value:visibleSky},rotation:{value:-1.2},intensity:{value:1.03}},
    ...
    fragmentShader:`uniform sampler2D panorama;uniform float rotation;uniform float intensity;varying vec3 skyDirection;
      ${skySamplingGLSL}
      void main(){gl_FragColor=vec4(sampleFloridaSky(panorama,skyDirection,rotation,intensity,0.0),1.0);
