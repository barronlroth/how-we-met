...
`npm run blender:hero` creates the original airboat and seated couple in `florida/assets/models/airboat-couple-v4.glb`. The source is `scripts/build-florida-hero.py` plus `scripts/florida_characters.py`; editable `.blend`, review renders, and geometry statistics go to ignored `artifacts/florida-characters-v4/`. The selected animated-feature concept guides the faces, hair, and outfits. A separately generated painted face atlas in `art/characters/` is embedded as one compact 1024×512 JPEG; the original supplied portraits are not embedded or shipped. UV-mapped facial paint carries the eyes, brows, smile, and stubble on continuously sculpted heads. Eyes have separate shallow glossy surfaces; the heads, hair, body, clothing, and boat remain real geometry. The script creates geometry and shared PBR materials, with independent Fan, Nina, Barron, and PointingArm nodes. Fixed geometry is merged per material within each articulation node.

`tests/hero-art.test.mjs` loads the exported GLB with Three.js and verifies the animation nodes, axis convention, collision/camera envelope, and budgets of 42 meshes, 65,000 triangles, 25 exported materials, and 1.8 MB. Runtime performance still uses the full-route browser benchmark.
