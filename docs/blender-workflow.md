...
`npm run blender:hero` creates the original airboat and seated couple in `florida/assets/models/airboat-couple-v5.glb`. The source is `scripts/build-florida-hero.py` plus `scripts/florida_characters.py`; editable `.blend`, review renders, and geometry statistics go to ignored `artifacts/florida-characters-v5/`. The selected animated-feature concept guides the faces, hair, and outfits. A separately generated painted face atlas in `art/characters/` is embedded as one compact 1024×512 JPEG; the original supplied portraits are not embedded or shipped. The v5 shape correction preserves that source atlas and uses a nearly uniform face UV scale so the nose-to-mouth region no longer stretches relative to the eyes. Smaller heads, shorter lower faces, broader jaws/cheeks, lowered neck seating, integrated facial relief and convex almond eye lenses align geometry with the painted landmarks. Directional curls and broad tapered blonde waves replace the previous hair masses. Shirts join shoulders and sleeves into one surface, shorts join hips and seated thighs, and tapered limbs end in palms, thumbs and fingers. Nina remains blonde. The heads, hair, body, clothing, and boat are real geometry; expressions are fixed, with no facial rig. The script creates geometry and shared PBR materials, with independent Fan, Nina, Barron, and PointingArm nodes. Fixed geometry is merged per material within each articulation node.
