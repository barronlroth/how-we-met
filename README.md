...
- A passing summer squall darkens the mangrove cut, with windblown rain and a cool overcast sky reflected in the water. It eases in after the marina and clears before party cove. Rain uses one fixed GPU buffer (1,200 streaks in Detailed, 600 in Smooth, 360 with reduced motion), no shadow/reflection/AO rendering, and freezes when paused. Steering and visibility of nearby hazards remain unchanged; no lightning flashes.
...
F2 toggles local frame-rate, frame-pacing, draw-count, and per-sector diagnostics; it records only while visible. For repeatable comparisons, use the same viewport and `?benchmarkDpr=1.25` (or `1`). `benchmarkAA=2` or `4` permits controlled sample-count comparisons; F2 reports the actual value. Add `&benchmarkFire=1` and choose **Watch a run** to stress-test continuous water-cannon fire along the demo route; player-controlled races are unaffected. Detailed caps device density at 1.25; Smooth caps it at 1, halves geometry samples and shadow-map resolution, and skips ambient occlusion. `scripts/build.mjs` preserves Toronto and bundles Three.js plus Florida into `dist/`.
